...
def sync_comics():
    ...
    def generate():
        try:
            ...
            comics = {}  # comic_name -> list of (chapter_name, file_path, format)
            for chapter_file in chapters:
                comic_name = chapter_file.parent.name
                chapter_name = chapter_file.stem  # filename without extension
                file_format = chapter_file.suffix[1:].upper()  # e.g., PDF, CBZ
                if comic_name not in comics:
                    comics[comic_name] = []
                comics[comic_name].append((chapter_name, str(chapter_file), file_format))
